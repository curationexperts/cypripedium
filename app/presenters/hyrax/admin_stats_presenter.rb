...
module Hyrax
  class AdminStatsPresenter
    ...
    def extract_date_from_stats_filters(key:, as_of:)
      start_date_str = stats_filters['start_date']
      end_date_str = stats_filters['end_date']

      if start_date_str.blank?
        pre_quarter = Time.zone.now - 3.months
        start_date = pre_quarter.beginning_of_quarter
        end_date = pre_quarter.end_of_quarter
      elsif end_date_str.blank?
        start_date = Time.zone.parse(stats_filters['start_date'])
        end_date = Time.zone.now
      else
        start_date = Time.zone.parse(stats_filters['start_date'])
        end_date = Time.zone.parse(stats_filters['end_date'])
      end

      if key == :start_date
        start_date.public_send(as_of)
      else
        end_date.public_send(as_of)
      end
    end
